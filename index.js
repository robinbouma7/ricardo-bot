...
bot.on('ready', () => {
	...
	console.log(date.toLocaleTimeString('nl-NL', {
...
bot.on('message', msg => {
	...
	if (!msg.author.bot) {
		...
		if (msg.guild === null) {
			...
			console.log("voorkomen dat degene die gemute us dit bericht stuurde:", msg.content);
			msg.delete();
		}
			...
			if (msg.channel.name == "ja") {
				...
				if (msg.content.includes("Nee") || msg.content.includes("nee") || msg.content.includes("nEE") || msg.content.includes("nEe") || msg.content.includes("neE") || msg.content.includes("NEE")) {
					...
					console.log("user was:", msg.author.username)
					msg.delete();
				}

			}
			else if (msg.channel.name === "pokemon") {
				//moet nog maken dat je pokemon bericht embeds kan maken
				if (msg.content.startsWith("ric embed") || msg.content.startsWith("Ric embed")) {
					let image = msg.content.slice(10).split(' ');
					msg.delete();
					if (image.length > 0) {
				...
				if (msg.guild.id === "865909031460405259") {
					...
					const helpEmbedr = new Discord.MessageEmbed()
						...
						.setColor('#0011ff')
						.setDescription(
							'`ric update me algemeen` \nkrijg algemene updates\n\n `ric update me project` \nkrijg updates van alle ricardogames projecten\n\n`ric update me bot`\nkrijg updates van development van een epic bot\n\n`ric im a speedrunner`\nkrijg de speedrunner rol'
						)
						.setFooter('ricardo is epic chad')
						.setThumbnail(
							'https://cdn.discordapp.com/avatars/833605715771654174/56529ef670666520dcbf70e7a6e4b5ad.webp'
				...
				if (msg.channel.nsfw) {
					...
					if (geil_rng === 7) {
						...
						//realistic ric
						msg.channel.send("https://cdn.discordapp.com/attachments/833624751208660992/897480908431573072/image0.jpg");
					}
					else if (geil_rng < 5) {
						//ric 2.0
						msg.channel.send("https://cdn.discordapp.com/attachments/833624751208660992/897482151958835250/ricardo-updated.png");
					}
					else {
						//ric klassiek
				...
				msg.channel.send(` ${breadfront} ${breadmid} ${breadmid} ${breadmid} ${breadend}`);
			...
			} else if (msg.content.toLowerCase() === 'ric update me project' || msg.content.toLowerCase() === 'ric update me projecten') {
				if (msg.guild.id === '816263795613761597') {
					...
					if (msg.member.roles.cache.has('833624859464433684')) {
						...
						msg.reply('je krijg nu geen updates meer voor ricardogames projecten.');
						console.log(`project rol weggenomen van: ` + msg.author.username);
					} else {
						let member = msg.member;
						member.roles.add(pspupdates).catch(console.error);
						msg.reply('je krijg nu updates voor alle ricardogames projecten!');
						console.log(`projecten rol toegevoegd aan: ` + msg.author.username);
					}
				} else {
					//niks
				}
			} else if (msg.content.toLowerCase() === 'ric update me algemeen') {
				if (msg.guild.id === '816263795613761597') {
					...
					if (msg.member.roles.cache.has('833625075017449482')) {
						...
						msg.reply('je krijg nu geen algemene updates meer.');
						console.log('');
						console.log(`algemeen rol weggenomen van: ` + msg.author.username);
					} else {
						let member = msg.member;
						member.roles.add(webupdates).catch(console.error);
						msg.reply('je krijg nu algemene updates!');
						console.log('');
						console.log(`algemeen rol toegevoegd aan: ` + msg.author.username);
					}
				} else {
					//niks
				}
				...
				if (args.join(' ').length > 0) {
					var tosay = args.join(' ').toLowerCase();
					console.log(`ric moet zeggen: ${tosay}`);
					if (tosay === "i’m gay" || tosay === "im gay" || tosay === "i'm gay") {
						msg.reply("we know.");
					}
					else {
						msg.delete();
		...
		if(msg.channel.name === "ja" && !message.includes("ja")) {
			msg.delete();
			console.log("geen ja van dad verwijderd.");
		}
	}
	else if(msg.author.bot && msg.channel.name == "ja") {
		msg.delete();
	}
